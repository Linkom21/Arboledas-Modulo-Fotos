...
@Component({
  selector: 'app-qys-qsp-abiertas',
  templateUrl: './qys-qsp-abiertas.page.html',
  styleUrls: ['./qys-qsp-abiertas.page.scss'],
})

export class QysQspAbiertasPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    await this.serviceUsuarios.get(val[4],val[1],val[2],val[3]).subscribe(async user=>{
      ...
      //console.log(usuarios.id);
        await this.serviceQySAbiertas.getQuejasCerradasAbiertas(usuarios.id_residencial,val[1],val[2],val[3]).subscribe(response=>{
          this.queja=(response);
  ...
  detalle(quejas : Quejas){
    //this.navCtrl.navigateForward('/qys-detalle-abierta/1');
